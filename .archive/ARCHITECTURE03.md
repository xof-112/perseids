...
**\* Namens-Hinweis:** "Spread" taucht sowohl in Block 5 (Stereo-Streuung einzelner Grains) als
auch in Block 8 (Breite der Pan-Drift-Wanderung) auf — zwei unterschiedliche Parameter mit
gleichem Namen in unterschiedlichem Kontext. Funktional kein Problem (unterschiedliche
CycleRows), aber beim Display-Text ggf. verwirrend. Siehe Abschnitt 7 (Offene Punkte).
...
- Pan-Drift: pro Flare unabhängiger LFO (Dreieck/Sine-Mischung + leichter Jitter),
  Constant-Power-Panning. CycleRow Block 8: Spread (Phasenversatz zwischen den
  LFOs der Flares), Amplitude (Auslenkung), Velocity (Geschwindigkeit)
...
- **"Spread" doppelt vergeben** (Block 5 Grain-Streuung vs. Block 8 Pan-Drift-Phasenversatz) —
  funktional kein Problem, aber ggf. für Display-Klarheit umbenennen (z.B. Block 8 → "Fächer"
  oder "Phase")
