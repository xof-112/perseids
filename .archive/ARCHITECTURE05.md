...
| 2 | **Time** | Buffer (= Ringbuffer-Länge/max. Aufnahmedauer pro Flare), Hold (bis 30s, darüber = infinite/Default), Fade In, Fade Out |
...
**Block-9-Detail (Crossfade über 5 Flares):** Eine Amplituden-Welle (Lautstärke-Fokus) wandert
kontinuierlich vorwärts/rückwärts durch die aktiven Flares und blendet benachbarte Flares
...
(0 % = keine Wirkung, alle Flares gleichberechtigt; 100 % = nur die fokussierte Flare voll
hörbar). Nicht-fokussierte Flares werden also im Normalfall nur ABGESCHWÄCHT, nicht
...
auf derselben VCA-Stufe wie Flare-Level, also NACH den Pre-Fader-Abgriffen — Regel 2.5 bleibt
unberührt, Mod-Matrix und Sends sehen die Welle nicht. Die Welle läuft nur über die laut
Block 1 aktiven Flares. Solo übersteuert die Welle (Solo-Flare bleibt voll hörbar);
...
| Lang, allein | **Reset-Abfrage:** Display zeigt „Alle Flares löschen?" — ein erneuter Kurzdruck innerhalb 3 s bestätigt und löscht alle Flares; Timeout oder Reglerbewegung bricht ab. Während der Abfrage gilt Kurzdruck als Bestätigung, NICHT als Play/Pause |
...
- **Buffer** (Block 2): Länge des SDRAM-Ringbuffers pro Flare = maximale Aufnahmedauer
...
(kürzer/langsamer = näher am Ausklingen), Reset-Bestätigungsdialog („Alle Flares
löschen?", siehe 4.7).
...
- **Age (Alter der Aufnahme):** lineare Hüllkurve über die Lebensdauer einer Flare — startet
  bei 0 % im Moment der Aufnahme und steigt auf 100 %, je näher die Flare ihrem Ende
...
**Quell-Flare:** Bei bis zu 5 simultanen Flares liefert die **jüngste nicht-gelockte
aktive Flare** den Age-/Pitch-Wert. Fallback: Sind alle aktiven Flares gelockt, liefert
die jüngste aktive Flare (unabhängig von Lock). Existiert keine aktive Flare, ist der
Wert 0 — ein reiner Ruhezustand ohne praktische Relevanz, da es dann ohnehin nichts
Hörbares gibt, das moduliert werden müsste. Regel 2.5 gilt trotzdem: Eine auf Level 0
stehende, aber weiterhin aktive Flare bleibt als Age-/Pitch-Modulationsquelle nutzbar.
...
- Crossfade: wandernde Amplituden-Welle über die aktiven Flares gemäß 4.1
  ...
  Slew-Rate-Limiting (BBD-artig) beim Round-Robin-Ersetzen einer Flare
...
ParameterRegistry, alle Blöcke 1–11 sowie Flare-Level), Divider.
```
...
- **~~Auto-Mod-Quell-Flare~~ Erledigt:** jüngste nicht-gelockte aktive Flare (Fallback siehe 4.10)
