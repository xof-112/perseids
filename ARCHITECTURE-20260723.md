...
6. **Non-blocking ADC mux polling in the main loop**, never in the audio callback. Smooth
   ...
   **Note:** The Daisy Seed only has 12 native ADC pins (confirmed via Electrosmith
   documentation) — given our channel count (19 pots — the 4 mod-amplitude pots among them
   act as bipolar attenuverters, see 4.3 — plus CV inputs), external mux ICs (e.g. CD74HC4067,
...
| Trail Level | 5 | Pot with push | Turn = this Trail's level; short press = Lock; long press = Solo |
...
**Total: 19 pots + 1 encoder + 2 buttons.**
...
| 2 | **Time** | Buffer (= ring buffer length/max. recording time per Trail), Hold (up to 30s, beyond that = infinite/default), Fade In, Fade Out |
...
| 11 | **Multi** (Encoder) | Global Dry/Wet, Macro1, Macro2, Settings |
...
- Hold countdown up to 30s, then fade-out; values >30s = infinite (also the
  boot default)
